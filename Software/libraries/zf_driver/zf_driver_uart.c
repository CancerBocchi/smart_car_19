...
void uart_set_handle(uart_index_enum  uartn, lpuart_handle_t *handle, lpuart_transfer_callback_t callback, uint8 *tx_buff, uint32 tx_count, uint8 *rx_buff, uint32 rx_count)
{
    LPUART_TransferCreateHandle(uart_index[uartn], handle, callback, NULL);
    //设置接收缓冲区与发送缓冲区
